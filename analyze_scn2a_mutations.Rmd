...
It shows that the `d` object is '__data.frame___'. 
...
Right. It's '__"character"__'. How about the class of the column `Effect`?
...
> Q. When is the earliest day for having seizure? 알려고
> min(d1$SeizureOnsetDays) 해보면
Error: object 'd1' not found
> character라서 이렇게 나옴. sorting하려면 numeric으로 바꿔줘야.
...
```{r}
d1[is.na(d1$SeizureOnsetDays2),]$SeizureOnsetDays # 하거나

d1$SeizureOnsetDays[is.na(d1$SeizureOnsetDays2)] # 둘다 ok.
```
...
tail(d1$SeizureOnsetDays)
tail(d1$SeizureOnsetDays3) #7th row부터 <가 있었어가지고 1-6th row에는 <있고없고 차이가 안보이지만 원래는 이렇게 확인! 항상 확인해보고 넘어가는 습관들이기!
```
...
```{r}
min(d1$SeizureOnsetDays3) # earliest day
max(d1$SeizureOnsetDays3) # latest day
```
...
During these exercises, we will look at the concept of `for-loop` and `vectorization`, which you learn from the Chapter 4.4 and 4.5. Let's look at the column `p.Protein`. It contains protein positions from each patient. What would you check at the first place? Write down your code to explore this column. 
...
Then, I would remove the characters from the string so we can have only numerics for positions. Here I use `gsub` function to extract numerics from string. Remove non-numeric characters from the string.
...
gsub('[^A-Z]','','p.R102X')
gsub('[^a-z]','','p.R102X')
#아래에 있는 것들은 그냥 연습으로 해봄.
#gsub에서 앞에 pattern부분''안에 ^넣으면 그거를 대체하는게 아니라 그것만 남기고 나머지를 대체!
```
...
```{r}
myfunction <- function(
   p.Protein = gsub('[^0-9]', '', d[i , 'p.Protein'])
   print(p.Protein) )
x <- 1:263
sapply(x, myfunction)
```
